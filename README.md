...
- 7-17 到达极限判定（`analysis/rung4_scan.py`、`plots/18_*.png`）：理想开关的物理地板
  100→400 为 1.55~1.61s（200→400 约 1.2~1.35s，元件蓄热 ≈80° 只能滑完）；统一律价格表
  首超 ≤3°→2.6s、4.4°→2.3s、5.4°→2.1s、6.5°→1.9s（标称暖炉），速度旋钮是 wr；
  MV 上限（真机冲刺峰值仅 53~58%，60→90 仿真无增益）与 lrK（v 空间恒 1，改了 = 让
  ESO 记假账）都不是速度旋钮；90% 满幅 380~425°C 实测 g≈0.80，孪生 v3 该段外推偏悲观。
